...
void setup() {
  ...
  adc->adc0->setAveraging(0);                                            // Set averaging (optional)
  ...
  adc->adc1->setAveraging(0);                                            // Set averaging (optional)
  ...
  //take the reference phasor
      std::vector<double> result = calibrate(200);
    ...
    u8g2.clearBuffer();  // clear buffer for storing display content in RAM
      ...
      u8g2.drawBox(125, 64/NUM_ITEMS * item_selected, 3, 64/NUM_ITEMS); 
        ...
        { 
          ...
          std::vector<double> result = calibrate(300);
...
std::vector<double> phaseShift(float* transmitter_coil, float* receiver_coil, int N, int sample_rate, float waveFrequency) {
  ...
  RR = (R*R2 + I*I2)/(R*R + I*I);
  II = (I2*R - R2*I)/(R*R + I*I);
  double shift = atan2(II, RR)*(180.0/ PI);
  double amplitude = 100*sqrt(II*II + RR*RR);
